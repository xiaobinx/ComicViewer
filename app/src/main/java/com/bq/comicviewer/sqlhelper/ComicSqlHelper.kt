...
class ComicSqlHelper(context: Context) : SQLiteOpenHelper(context, "comic", null, version) {
     ...
     */
    fun saveComic(comic: Comic) = writableDatabase.transaction {
     ...
     * 以ID <插入> 或 <更新> 一个历史纪录
     */
    fun updateHistory(id: Long, i: Int) = writableDatabase.transaction {
        ...
        if (1 > eff) {
            this.execSQL("INSERT INTO history(cid, i, read_date) VALUES(?, ?, ?)", arrayOf(id, i, date))
        }
    ...
    fun queryAndFillComic(comic: Comic) = readableDatabase.transaction {
        // 1.尝试查询ID 不存在就直接返回
        if (comic.id < 0) {
    ...
    fun queryHistory(p: Int, pageItem: PageItem): ArrayList<Comic> {
        ...
        db.rawQuery(
            "SELECT _id, url, cover_url, title, type, i FROM comic c LEFT JOIN history h ON c._id = h.cid ORDER BY read_date DESC LIMIT ?, ?"
            , arrayOf(start.toString(), pageItem.rowOfPage.toString())
        ).eachRow {
            list.add(
                Comic(
        ...
        db.rawQuery(
            "SELECT _id, url, cover_url, title, type, i FROM comic c LEFT JOIN history h ON c._id = h.cid ORDER BY read_date DESC",
            null
        )
            .println()
        return list
