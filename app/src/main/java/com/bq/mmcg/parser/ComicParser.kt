...
class ComicParser : IComicPageParser {
...
//        val title = parseTitle(html)
    ...
    private val patternList = Pattern.compile(
            "<a class=\"aRF\" target=\"_blank\" href=\"(.+?)\" ><img class=\"aRF_Scg\" border=\"0\" src=\"(.+?)\" width=\"250\" height=\"170\" alt=\"(.+?)\" onerror=\"this.onerror=null\";   \"this.src='.+?'\"  link=\"(.+?)\" ></img></a>")

    fun parseComics(html: String): ArrayList<Comic> {
        ...
        while (m.find()) {
            ...
            val comic = Comic(url, coverUrl, title, "comic")
            list.add(comic)
        }
